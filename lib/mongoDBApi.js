...
// const contactsCollectionName = process.env.DB_CONTACTS_COLLECTION;

const user = process.env.mongodb_db_user;
const password = process.env.mongodb_db_pass;
const host = process.env.mongodb_db_host;
const DbName = process.env.mongodb_db_name;
const uri = `mongodb+srv://${user}:${password}@${host}/${DbName}?retryWrites=true&w=majority`;
const contactsCollectionName = process.env.mongodb_db_contacts_collection;
...
export async function addContact(contactData) {
    try {
        // const client = await MongoClient.connect(uri);
        // const db = client.db(`${DbName}`);
        // const contactsCollectionName = db.collection(`${contactsCollectionName}`);
        const [client, contactsCollection] = await getClientAndCollection(contactsCollectionName);
        const newContactData = {...contactData}; // Makes a real copy, as it was previously modifying the original object
        const result = await contactsCollection.insertOne(newContactData);
        await client.close();
        return {id: result.insertedId.toString(), ...contactData};
    } catch (error) {
        throw new Error(error.message || 'Unable to create the contact.');
    }
}
